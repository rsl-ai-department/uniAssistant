...
export const Search: FC<SearchProps> = ({ onSearch, onAnswerUpdate, onDone }) => {
  ...
  const handleStream = async (sources: Source[]) => {
    try {
      const prompt = endent`Предоставь ответ на 2-3 предложения используя приведенные источники. Будь оригинален, точен и полезен. Цитируй источники в формате [1] или [2] или [3] после каждого предложения (не только в конце) для подкрепления своего ответа (Например: Правильно: [1], Правильно: [2][3], Неправильно: [1, 2]).
    ...
    <>
      {loading ? (
        ...
        <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center space-y-6 px-3 pt-32 sm:pt-64">
          ...
          >
            {showSettings ? "Убрать" : "Показать"} настройки
          </button>
